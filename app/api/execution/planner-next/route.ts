    ...
    const result =
      await runWithUserContext(
        ...
        async () => {
          ...
          if (!nextTask) {
            return {
              ...
              code:
                "PLAN_COMPLETE",
              message:
                "All tasks in the current Planner queue are complete.",
              outcomeId:
                outcome.id,
              outcomeTitle:
                outcome.title,
          ...
          const job =
            await jobStore.createExecutionJob(
              {
                goal:
                  `Execute Planner task: ${nextTask.title}`,
                planId,
                taskId:
                  nextTask.id,
                input,
          ...
          await jobStore.markExecutionJobRunning(
            job.id
          );
           ...
           *
           * 如果当前 Planner Task 明确要求读取
           * Founder 授权的 GitHub 文件，则直接进入
           * GitHub Read Bridge。
           *
           * 这里不把 GitHub READ 当作普通 LLM Runtime
           * 请求，避免模型“猜测”仓库内容。
           */
          const githubRead =
          ...
          if (
            ...
            if (
              ...
              const failed =
                await jobStore.markExecutionJobFailed(
                  job.id,
                  githubRead.error ??
                    "Planner GitHub read failed."
                );
            ...
            const githubResult =
              [
                `GitHub READ completed successfully.`,
                `Path: ${githubRead.path ?? "unknown"}`,
                `SHA: ${githubRead.sha ?? "unknown"}`,
                `Size: ${githubRead.size ?? githubContent.length}`,
                "",
                githubContent,
              ].join("\n");

            const completedJob =
              await jobStore.markExecutionJobCompleted(
                job.id,
                githubResult
              );
          ...
          /*
           * 非 GitHub READ 任务继续走原有 Runtime。
           *
           * 因此 C141.11.2-B 不破坏现有普通 Planner
           * Execution 能力。
           */
          const runtime =
