...
async function readJson(
  ...
  if (
    ...
    throw new Error(
      data.error ||
        `Outcome 操作失败（HTTP ${response.status}）。`
    );
  }
...
export default function OutcomesPage() {
  ...
  async function updateOutcomeStatus(
    ...
    try {
      ...
      setSuccessMessage(
        `「${outcome.title}」已更新为${getStatusLabel(
          status
        )}。`
      );
