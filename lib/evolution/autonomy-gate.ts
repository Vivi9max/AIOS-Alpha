...
import {
  getWebIntelligenceHealth,
} from "@/lib/web-intelligence/health";

export type AutonomyGateLevel =
  | "blocked"
  | "observation"
  | "autonomous";
...
export interface AutonomyGateResult {
  ...
  checks: {
    ...
    webIntelligenceFresh: boolean;
    webIntelligenceVerified: boolean;
    webIntelligenceSourceCount: number;
    webIntelligenceIndependentHosts: number;
  };
  candidateTask: {
    id: string;
    title: string;
...
const MIN_AUTONOMY_HEALTH_SCORE =
  90;

const WEB_INTELLIGENCE_MAX_AGE_MS =
  24 * 60 * 60 * 1000;
...
export async function evaluateAutonomyGate(): Promise<AutonomyGateResult> {
  ...
  const webHealth =
    await getWebIntelligenceHealth();
  ...
  if (
    ...
    blockers.push(
      "Persistent Redis storage is not ready.",
    );
  ...
  const runtimeSnapshot =
    targetUserId
      ? await runWithUserContext(
          targetUserId,
          async (): Promise<AutonomyRuntimeSnapshot> => {
            const heartbeat =
  ...
  if (!heartbeatAvailable) {
    blockers.push(
      "No Evolution Heartbeat result is available.",
    );
    ...
    blockers.push(
      "The latest Evolution Heartbeat is stale.",
    );
  ...
  const webIntelligenceAvailable =
    webHealth !== null;

  const webIntelligenceFresh =
    webHealth !== null &&
    timestamp -
      webHealth.timestamp <=
      WEB_INTELLIGENCE_MAX_AGE_MS;

  const webIntelligenceVerified =
    webHealth?.success === true &&
    webHealth.verified === true;

  const webIntelligenceSourceCount =
    webHealth?.sourceCount ?? 0;

  const webIntelligenceIndependentHosts =
    webHealth?.independentHosts ?? 0;

  /*
   * Web Intelligence is a capability-health signal,
   * not a global autonomy blocker.
   *
   * Non-web tasks may continue to run when the web
   * capability is unavailable. Web-dependent execution
   * must inspect this state before relying on fresh
   * external evidence.
   */
  if (
    !webIntelligenceAvailable ||
    !webIntelligenceFresh ||
    !webIntelligenceVerified
  ) {
    recommendations.push(
      "Web Intelligence health is unavailable or stale. Web-dependent tasks must re-verify external evidence before execution.",
    );
  }
  ...
  const decision: AutonomyGateDecision =
    ready
      ? "execute-one"
      : heartbeatAvailable
        ? "observe"
  ...
  const reason =
    ready
      ? "All core autonomy checks passed. Exactly one eligible Planner task may be dispatched. Web Intelligence health is exposed separately as a capability signal."
      : decision === "observe"
  ...
  return {
    ...
    checks: {
      ...
      webIntelligenceSourceCount,

      webIntelligenceIndependentHosts,
    },
