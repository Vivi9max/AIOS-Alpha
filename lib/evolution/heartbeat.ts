...
const TARGETS_KEY =
  "aios:evolution:targets";

const HEARTBEAT_INDEX_KEY =
  "aios:evolution:heartbeat-index";
...
function buildInsights(
  ...
  memory: Awaited<
    ReturnType<
      typeof getPersistentExecutionMemory
    >
  >,
  outcomes: Awaited<
  ...
  >,
  tasks: Awaited<
    ReturnType<typeof listPersistentTasks>
  >,
): EvolutionInsight[] {
  const cutoff =
    Date.now() -
  ...
  if (
    ...
    insights.push({
      ...
      observation:
        `${executionFailures.length} execution failure event(s) were recorded in the last 7 days.`,
      recommendation:
        "Prioritize retry/recovery verification before increasing autonomous execution volume.",
      evidence: {
        recentFailures:
          failures.length,
        executionFailures:
  ...
  if (
    ...
    insights.push({
      ...
      category:
        "planner",
      title:
        "Planner queue is executable",
      observation:
        "An active Outcome has executable todo tasks and no task is currently running.",
      recommendation:
        "Heartbeat may safely dispatch one eligible Planner task through the bounded autonomous runtime.",
      evidence: {
        activeOutcomes,
        todoTasks,
        doingTasks,
  ...
  if (
    ...
    insights.push({
      ...
      category:
        "execution",
      title:
        "Stable execution streak",
      observation:
        "Recent execution memory contains no failure events.",
      recommendation:
        "Preserve the current runtime path and increase automation only through verified, reversible steps.",
      evidence: {
        recentCount:
          recent.length,
        failures: 0,
  ...
  if (
    ...
    insights.push({
      ...
      category:
        "memory",
      title:
        "Evolution baseline not established",
      observation:
        "No persistent execution-memory events are available yet.",
      recommendation:
        "Continue recording execution, verification and recovery events before enabling more autonomous actions.",
      evidence: {
...
export async function runEvolutionHeartbeat(
  userId: string,
): Promise<EvolutionHeartbeatResult> {
  return runWithUserContext(
    ...
    async () => {
      ...
      let autonomousExecution:
        EvolutionHeartbeatResult["autonomousExecution"] =
        {
          attempted: false,
          executed: false,
          success: false,
          ...
          lifecycleId: null,
          message:
            "No autonomous task was dispatched.",
        };
            ...
            autonomousExecution =
              {
                ...
                message:
                  ...
                  Error
                    ? error.message
                    : "Autonomous task dispatch failed.",
              };
          }
        }
      }
      ...
      const nextAction =
        autonomousExecution.attempted
          ? autonomousExecution.success
            ? "Heartbeat completed one bounded autonomous task. Observe the resulting evidence before dispatching another task."
            : autonomousExecution.status ===
                "blocked"
              ? "Autonomous execution was blocked by the Safety Gate. Preserve the task and wait for the next eligible cycle."
              : "Autonomous execution did not complete successfully. Preserve the task for verified recovery before increasing autonomy."
          : recentFailures >
              0
...
export async function registerEvolutionTarget(
  userId: string,
) {
  const targets =
    (await storage.get<
