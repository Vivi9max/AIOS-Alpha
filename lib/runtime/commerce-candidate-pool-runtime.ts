...
function normalizeCandidate(
  ...
  return {
    ...
    commercialSignalScore: clamp(
      Number(input.commercialSignalScore ?? 0),
    ),
  };
}
function buildProductIntelligence(
  ...
  const demonstration = unique(
    candidate.demonstration ?? [],
    8,
  );
  return {
    success: true,
    code: "C145_1_COMMERCE_PRODUCT_INTELLIGENCE_PASS",
    product: {
      name: candidate.name,
    ...
    sellingPoints,
    targetCustomer: "待进一步验证",
    marketingPattern: {
      hook: "待进一步验证",
      demonstration:
        demonstration.join("、") || "待进一步验证",
      emotionalTrigger: "待进一步验证",
      purchaseTrigger: "待进一步验证",
    },
    evidence: [],
    confidence: {
      product: 100,
      sellingPoints:
        sellingPoints.length > 0 ? 70 : 0,
      price: 0,
      commercialSignal: clamp(
        Number(candidate.commercialSignalScore ?? 0),
      ),
    },
    unknowns: [
    ...
    source: {
      type: "video-vision",
      model: "C145.4 candidate-pool input",
    },
  };
}
function scoreCandidate(
  ...
  const priceScore =
    priceFound
      ? market.priceSignals.length > 0
        ...
        : 50
      : 0;
  // Competition is treated as evidence availability,
  // not as an objective market-wide competition claim.
  const competitionScore =
    competitorFound ? 100 : 0;
  const contentScore = clamp(
    product.confidence.commercialSignal,
  );
  ...
  const evidenceCompleteness = clamp(
    (
      completenessParts.filter(Boolean).length /
  ...
  return {
    ...
    candidate: {
      ...
      demonstration:
        product.visualSignals.demonstration,
    ...
    priceSignalScore: priceScore,
    supplySignalScore: supplyScore,
    competitionSignalScore: competitionScore,
    contentSignalScore: contentScore,
    verified:
      market.verification.overallVerified,
    priority,
    unknowns,
...
async function processOneCandidate(
  candidate: CommerceCandidateInput,
): Promise<CommerceCandidateResult> {
  const normalized =
    normalizeCandidate(candidate);
  const product =
    buildProductIntelligence(normalized);
  try {
    const market =
    ...
    const fallbackMarket: CommerceMarketIntelligence = {
      success: false,
      code:
...
export async function executeCommerceCandidatePool(
...
): Promise<CommerceCandidatePoolResult> {
  ...
  const results: CommerceCandidateResult[] =
    [];
  for (
    let index = 0;
