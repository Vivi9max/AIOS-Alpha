...
import type { VideoMediaType } from "./video-media-runtime";
...
const SAMPLE_RATIOS = [0, 0.25, 0.5, 0.75, 0.95];
...
function validateMediaUrl(rawUrl: string): URL {
  ...
  if (isPrivateIpv4(hostname)) {
    throw new Error("Private IPv4 media URLs are blocked.");
  }

  return parsed;
}

async function checksum(data: Buffer): Promise<string> {
...
async function getDecoderVersion(
  ...
  try {
    ...
    const match = combined.match(/ffmpeg version\s+([^\s]+)/i);
...
async function resolveDecoder(
  ...
  if (environmentPath) {
    const version = await getDecoderVersion(environmentPath);
  ...
  if (ffmpegStatic) {
    const version = await getDecoderVersion(ffmpegStatic);
  ...
  const systemVersion = await getDecoderVersion("ffmpeg");
...
async function downloadVideo(
  ...
  if (contentLengthHeader) {
    ...
    if (
      ...
      contentLength > MAX_VIDEO_DOWNLOAD_BYTES
    ) {
      throw new Error(
  ...
  if (!response.body) {
    throw new Error("Video response body is unavailable.");
  }
  ...
  if (totalBytes === 0) {
    throw new Error("Video download returned no bytes.");
  }

  const outputPath = join(workspace, "input-video");
  ...
  try {
    const result = await runCommand(
      ...
      [
        ...
        "error",
        "-ss",
        Math.max(0, timestampSeconds).toFixed(3),
        "-i",
        inputPath,
        "-frames:v",
        "1",
    ...
    const hash = await checksum(data);
...
export async function executeRuntimeVideoFrames(
  ...
  try {
    ...
    return {
      ...baseResult,
      success: false,
      code: "C144_7_VIDEO_FRAME_EXTRACTION_ERROR",
      decoder: {
        available: false,
      },
      frameCount: 0,
  ...
  if (!decoder) {
    return {
      ...baseResult,
      success: false,
      code: "C144_7_VIDEO_FRAME_DECODER_UNAVAILABLE",
      decoder: {
        available: false,
      },
      frameCount: 0,
  ...
  try {
    ...
    for (
      let index = 0;
      index < SAMPLE_RATIOS.length &&
      index < MAX_FRAMES;
      index += 1
    ) {
      const ratio = SAMPLE_RATIOS[index];

      const timestampSeconds = duration
        ? Math.min(
            duration,
            duration * ratio,
          )
    ...
    const successfulFrameCount =
      frames.filter((frame) => frame.success).length;

    const totalBytesRead = frames.reduce(
      (sum, frame) => sum + frame.bytesRead,
      0,
    );
    ...
    const imagesExtracted =
      successfulFrameCount === frames.length &&
      frames.length > 0;
    ...
    if (successfulFrameCount === frames.length && frames.length > 0) {
      code =
        "C144_7_VIDEO_FRAME_EXTRACTION_PASS";
    } else if (successfulFrameCount > 0) {
      code =
        "C144_7_VIDEO_FRAME_EXTRACTION_PARTIAL";
    } else {
      code =
    ...
    return {
      ...baseResult,
      success: successfulFrameCount > 0,
      code,
      decoder: {
        available: true,
        name: "FFmpeg",
