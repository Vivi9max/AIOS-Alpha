...
function buildObjectiveMarketPrompt(objective: CommercialObjective): string {
  return [
...
export async function executeLiveCommercialOpportunity(
  ...
  const objective = await getCommercialObjective(input.objectiveId);
  ...
  const prompt =
    ...
    buildObjectiveMarketPrompt(objective);

  const web = await retrieveWebEvidence(prompt);
  ...
  const decision = buildLiveDecision(web);

  if (
    !decision.success ||
    decision.verification?.verified !== true ||
    decision.evidence.length < 2 ||
    !normalizeText(decision.conclusion) ||
    !normalizeText(decision.nextStep) ||
  ...
  const runtime = await executeLiveCommercialRuntime({
    objectiveId: objective.id,
    decision,
  });

  if (!isLiveCommercialRuntimeReady(runtime)) {
    return buildBlockedResult(
      input,
      objective,
      "runtime-blocked",
...
export function isLiveCommercialOpportunityReady(
  ...
  if (!result.runtime) return false;

  return isLiveCommercialRuntimeReady(result.runtime);
}
...
export function buildLiveCommercialOpportunityContext(
  ...
  if (result.decision) {
    lines.push(
      ...
      `VERIFIED: ${
        result.decision.verification?.verified
          ? "YES"
          : "NO"
      }`,
